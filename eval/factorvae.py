...
def compute_factorvae_score(model,
                            train_size=10000,
                            test_size=5000,
                            batch_size=64):
    dsprites_loader = ColourDSpritesLoader(
        npz_path=
        './data/DSPRITES/dsprites_ndarray_co1sh3sc6or40x32y32_64x64.npz')
    train_data = ColourDSprites(size=train_size,
                                dsprites_loader=dsprites_loader,
                                batch_size=batch_size)
    test_data = ColourDSprites(size=test_size,
                               dsprites_loader=dsprites_loader,
                               batch_size=batch_size)
    ...
    train_votes = generate_training_batch(model, train_data, train_size,
                                          variances, active_dims)
    classifier = np.argmax(train_votes, axis=0)
    other_idx = np.arange(train_votes.shape[1])

    train_acc = np.sum(train_votes[classifier,
                                   other_idx]) * 1. / np.sum(train_votes)

    test_votes = generate_training_batch(model, test_data, test_size,
                                         variances, active_dims)
    test_acc = np.sum(test_votes[classifier,
                                 other_idx]) * 1. / np.sum(test_votes)
...
def generate_training_sample(model, data, variances, active_dims):
    ...
    x = torch.tensor(data.latent_to_observations(z),
                     dtype=torch.float32).to(CUDA).squeeze()
...
def generate_training_batch(model, data, num_points, variances, active_dims):
    votes = np.zeros((len(data.factors_sizes), variances.shape[0]),
                     dtype=np.int64)
    for _ in range(num_points):
        i, argmin = generate_training_sample(model, data, variances,
                                             active_dims)
        votes[i, argmin] += 1
    return votes
